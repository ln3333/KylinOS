...
case "$ID" in
    ...
    "alpine")
        ...
        ;;
    "centos"|"kylin")
        # For CentOS/Kylin, we skip checksum verification as nginx is built from source
        entrypoint_log "$ME: info: Skipping checksum verification for $ID"
        ;;
    *)
        entrypoint_log "$ME: info: Unsupported distribution: $ID, skipping checksum verification"
        ;;
esac
